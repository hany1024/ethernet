...
void setup() {
  ...
  Serial.println(F("Enter 1 to display web / Enter 0 to Just Read"));
  ...
  if(Mode == '1'){
    ...
    Serial.println(F("Sensor Read Mode......"));
    if (!SD.begin(SDCARD_CS)) {
        error("card.init failed!");
    }
    Serial.println(F("Card is initialized")); 
...
void loop()
{
  if(Mode == '1'){
    ...
    root = SD.open("LiDi.txt", FILE_WRITE);
    if(root){
      Serial.println(F("Writing to LiDi.txt...."));
      root.println("THIS BETTER WORK");
      root.close();
      Serial.println(F("Done"));
      delay(3000);
    }
    else{
      Serial.println(F("Error opening LiDi.txt"));
    }
  }
}
